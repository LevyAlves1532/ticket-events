    ...
    <main>
        <div class="max-w-5xl w-full mx-auto py-10 px-5">
            ...
            <div class="mb-5 flex justify-end">
                <x-button type="link" label="Adicionar Comprador +" to="{{route('ticket_buyers.create_event', ['id_event' => $event['id']])}}" />
            </div>

            <x-table :columns=$columns>
                <tr class="flex p-5">
                    <td class="flex-[2] {{$column_class}}">Lêvy P. Alves</td>
                    <td class="flex-[2] {{$column_class}}">(47) 9 9196-6719</td>
                    <td class="flex-1 {{$column_class}}">
                        <a href="{{route('ticket_buyers.delete_event', ['id_event' => 1])}}" class="group p-3 rounded-[999px] flex justify-center items-center hover:bg-red-400/25">

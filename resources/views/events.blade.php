    ...
    <main>
        <div class="max-w-5xl w-full mx-auto py-10 px-5">
            <x-header title="Eventos">
                <x-button type="link" label="Voltar" to="{{route('home')}}" />
                <x-button type="link" label="Eventos +" to="{{route('events.create')}}" />
            </x-header>
                ...
                <x-table :columns=$columns>
                    @foreach ($events as $event)
                        <tr class="flex p-5">
                            ...
                            <td class="flex-[2] {{$column_class}}">{{date('d/m/Y', strtotime($event->due_date))}}</td>
                            <td class="flex-1 {{$column_class}}">{{$event->qtd_tickets}}</td>
                            <td class="flex-1 {{$column_class}}">R$ {{number_format($event->price_ticket, 2, ',', '.')}}</td>
                            <td class="flex-1 {{$column_class}}">
                                <a href="{{route('events.update', ['id' => $event->id])}}" class="group p-3 rounded-[999px] flex justify-center items-center hover:bg-blue-400/25">
                                ...
                                <a href="{{route('events.delete', ['id' => $event->id])}}" class="group p-3 rounded-[999px] flex justify-center items-center hover:bg-red-400/25">

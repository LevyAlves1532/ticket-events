<x-layout>
    ...
    <main>
        <div class="max-w-5xl w-full mx-auto py-10 px-5">
            <x-header title="Compradores">
                <x-button type="link" label="Voltar" to="{{route('home')}}" />
                <x-button type="link" label="Comprador +" to="{{route('buyers.create')}}" />
            </x-header>
            ...
            @if ($buyers->count() > 0)
                <x-table :columns=$columns>
                    @foreach ($buyers as $buyer)
                        <tr class="flex p-5">
                            <td class="flex-[2] {{$column_class}}">{{$buyer->name}}</td>
                            <td class="flex-[2] {{$column_class}}">{{$buyer->phone}}</td>
                            <td class="flex-[1] {{$column_class}}">
                                <a href="{{route('buyers.update', ['id' => $buyer->id])}}" class="group p-3 rounded-[999px] flex justify-center items-center hover:bg-blue-400/25">
                                ...
                                <a href="{{route('buyers.delete', ['id' => $buyer->id])}}" class="group p-3 rounded-[999px] flex justify-center items-center hover:bg-red-400/25">
